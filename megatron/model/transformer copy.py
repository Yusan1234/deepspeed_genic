...
####################
###  SparseMoE Implementation
###################
#Expert module

class ParallelExpert(MegatronModule):
    ...
    def __init__(self, config, moe=False, enable_expert_tensor_parallelism=False):
        super(ParallelExpert, self).__init__()
        args = get_args()
        ...
        self.dropout = torch.nn.Dropout(config.expart_dropout)
...
#noisy top-k gating
class NoisyTopkRouter(MegatronModule):
...
class DeepSets(MegatronModule):
    ...
    def forward(self, expert_outputs, gating_scores):
        ...
        # ゲーティングスコアの次元を調整
        gating_scores = gating_scores.view(-1, 1).mean(dim=0, keepdim=True)
...
class SparseMoE(MegatronModule):
    ...
    def __init__(self, config):
        ...
        args = get_args()
        self.router = NoisyTopkRouter(config.hidden_size, args.num_experts_switch, args.top_k)
        self.experts = torch.nn.ModuleList([ParallelExpert(config.hidden_size) for _ in range(args.num_experts_switch)])

        self.deepsets = DeepSets(config.hidden_size, config.hidden_size, config.hidden_size)
